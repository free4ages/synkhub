...
class StarRocksBackend(SqlBackend):
    ...
    async def connect(self) -> None:
        """Connect to StarRocks database via DataStore"""
    ...
    async def disconnect(self) -> None:
        """Disconnect from StarRocks database via DataStore"""
        if self._datastore:
    ...
    async def execute_http_query(
        ...
        # Access the StarRocks datastore implementation directly for HTTP operations
        starrocks_impl = self._datastore._datastore_impl
        if not hasattr(starrocks_impl, 'execute_http_query'):
            raise RuntimeError("HTTP query execution not supported by this datastore")
        
        return await starrocks_impl.execute_http_query(sql, action, timeout)
