...
class DataFetchStage(BatchProcessor):
    ...
    def __init__(self, sync_engine: Any, config: Dict[str, Any] = None, logger=None, data_storage: Optional[DataStorage] = None):
        ...
        self.use_pagination = self.config.get('use_pagination', False)
    
    async def process_batch(self, batch: DataBatch) -> DataBatch:
        """Fetch data for the batch"""
        metadata = batch.batch_metadata
        
        if not metadata.get("needs_fetch", False):
            # No data fetching needed (e.g., for deletions)
            return batch
        ...
        try:
            if change_type in ["full_sync", "added", "modified"]:
