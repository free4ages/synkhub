...
class SyncEngine:
    ...
    async def sync(self, strategy_name: Optional[str] = None, start: Any = None, end: Any = None,
        ...
        try:
            ...
            pipeline_builder = PipelineBuilder(self, self.logger, self.data_storage)
            ...
            pipeline = pipeline_builder.build_pipeline_from_stages(self.stage_configs)
            
            self.logger.info(f"Executing pipeline with {self.config.max_concurrent_partitions} max concurrent partitions")
            ...
            # Finalize metrics
            progress_manager.finalize_metrics(change_detection_stats.get('total_partitions_processed', 1))
    ...
    async def create_backend(self, backend_config: Dict[str, Any], column_schema: Optional[ColumnSchema] = None):
