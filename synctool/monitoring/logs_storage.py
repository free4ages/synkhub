...
class LogsStorage:
    ...
    def __init__(self, logs_dir: str = "./data/logs", max_runs_per_strategy: int = 50):
    ...
    def _strategy_dir(self, job_name: str, strategy_name: str) -> Path:
        ...
        strategy_dir = self.logs_dir / job_name / strategy_name
        strategy_dir.mkdir(parents=True, exist_ok=True)
        return strategy_dir

    def _run_file(self, job_name: str, strategy_name: str, run_id: str) -> Path:
        """Get log file path for a specific run"""
        return self._strategy_dir(job_name, strategy_name) / f"{run_id}.log"
    ...
    def append_log(
        ...
        run_file = self._run_file(job_name, strategy_name, run_id)
        try:
            with open(run_file, "a", encoding="utf-8") as f:
                f.write(json.dumps(entry, ensure_ascii=False) + "\n")
        except Exception as e:
            self.logger.warning(f"Failed to append log for {job_name}:{strategy_name}:{run_id}: {e}")

        # Cleanup old runs for this strategy
        self._cleanup_old_runs(job_name, strategy_name)
    ...
    def get_run_logs(
        ...
        strategy_name: str = "default"  # New parameter with default
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Failed to read logs for {job_name}:{strategy_name}:{run_id}: {e}")
            return []
    ...
    def _cleanup_old_runs(self, job_name: str, strategy_name: str) -> None:
        """Keep only the latest N runs by modification time for a specific strategy."""
        strategy_dir = self._strategy_dir(job_name, strategy_name)

        run_files = sorted(
            strategy_dir.glob("*.log"),
